...
final class ChatViewController: JSQMessagesViewController, SessionIDDest  {
    ...
    private lazy var userIsTypingRef: FIRDatabaseReference = self.sessionRef!.child("typingIndicator").child(self.senderId)
    private lazy var usersTypingQuery: FIRDatabaseQuery = self.sessionRef!.child("typingIndicator").queryOrderedByValue().queryEqual(toValue: true)
